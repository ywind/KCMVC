...
public class Dispatcher extends HttpServlet {
	...
	private void initInterceptor() {
		...
		for (String uri : interceptorMap.keySet()) {
			 Collections.sort(interceptorMap.get(uri), new Comparator<Interceptor>() {
				@Override
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (parametersMap.size() == 0) {//如果参数为空
				...
				Method method =handlerExecutionChain.getHandlerAction().getMethod();
				...
				for (int i = 0; i < parametersNames.size(); i++) {
					try {
						parameters[i]=TypeConverter.convert(clazz[i],parametersMap.get(parametersNames.get(i))[0]);
					} catch (ParseException e) {
						response.sendError(400);
					}
				}
				result = handlerExecutionChain.execute(request,response,parameters);
			}
	...
	private HandlerExecutionChain getHandlerExecutionChain(String uri){
		...
		handlerExecutionChain.setInterceptorChain(new InterceptorChain(getInterceptors(uri)));;
